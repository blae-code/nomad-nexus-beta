...
export default function VerseClock() {
  const verseTime = useVerseTime();

  if (!verseTime) return null;
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900/50 border border-orange-500/30 rounded hover:border-orange-500/60 transition-colors">
      ...
      <div className="flex flex-col">
        <span className="text-xs font-mono font-bold text-orange-400">{verseTime.time}</span>
        <span className="text-[10px] text-zinc-500 font-semibold uppercase tracking-wider">{verseTime.date}</span>
      </div>
    </div>
  );
}