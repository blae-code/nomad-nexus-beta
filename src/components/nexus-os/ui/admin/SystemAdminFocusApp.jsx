        ...
        <div className="min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-2">
          <div className="min-h-0 lg:col-span-7 grid grid-rows-[auto_auto_auto_minmax(0,1fr)] gap-2">
            <section className="rounded-md border border-zinc-700/60 bg-zinc-900/55 p-2">
              <div className="flex items-center justify-between gap-2">
                <h4 className="text-[11px] font-semibold uppercase tracking-wide text-zinc-100">Workspace Launchers</h4>
                  ...
                  <div className="space-y-1">
                    {visibleDirectoryUsers.length ? (
                      visibleDirectoryUsers.map((entry) => (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <span className="inline-flex items-center gap-1.5 text-[11px] text-zinc-100 uppercase tracking-wide truncate min-w-0">
                              <NexusTokenIcon family="circle" color="cyan" size="sm" alt="User" />
                  ...
                  {visibleKeyRows.length ? (
                    visibleKeyRows.map((row) => (
                      <div key={row.id} className="rounded border border-zinc-800/70 bg-zinc-950/45 px-2 py-1.5">
                        <div className="flex items-center justify-between gap-2">
                          <span className="inline-flex items-center gap-1.5 text-[11px] text-zinc-100 font-mono truncate min-w-0">
                            <NexusTokenIcon 
                              family="hex" 
                              color={row.status === 'ACTIVE' ? 'green' : row.status === 'REDEEMED' ? 'cyan' : row.status === 'REVOKED' ? 'red' : 'yellow'} 
                              size="sm" 
                              alt={row.status} 
                            />
                            <span className="truncate">{maskKeyCode(row.code)}</span>
                          </span>
