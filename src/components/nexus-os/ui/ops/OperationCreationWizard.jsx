...
export default function OperationCreationWizard({
  ...
  return (
    <div className="nx-command-deck-backdrop" onClick={onClose}>
      <section className="nx-command-deck nexus-panel-glow max-h-[85vh] overflow-y-auto" onClick={(event) => event.stopPropagation()}>
        <header className="nx-command-deck-header sticky top-0 z-10 bg-zinc-900/95 backdrop-blur-sm">
          <div className="nx-command-deck-title-wrap">
            ...
            <div className="min-w-0">
              <h3 className="nx-command-deck-title">Operation Creation Wizard</h3>
              <p className="nx-command-deck-context">Configure and deploy new operations</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <NexusBadge tone={createPermission.allowed ? 'ok' : 'locked'}>{createPermission.allowed ? 'READY' : 'LOCKED'}</NexusBadge>
            <button type="button" onClick={onClose} className="nx-command-deck-close" title="Close wizard (Esc)">
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="nx-command-deck-content space-y-3 p-4">
      ...
      {errorText ? <div className="rounded border border-red-900/60 bg-red-950/30 px-2 py-1 text-xs text-red-300">{errorText}</div> : null}
        ...
        <footer className="sticky bottom-0 z-10 bg-zinc-900/95 backdrop-blur-sm border-t border-zinc-800 p-3">
