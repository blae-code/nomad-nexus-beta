                    ...
                    <div className="flex-1 min-h-0 overflow-hidden space-y-1">
                      {pagedMetrics.length > 0 ? (
                        pagedMetrics.map((metric, index) => {
                          ...
                          return (
                            <div key={`${metric.label}:${index}`} className="rounded border border-zinc-700/40 bg-zinc-900/40 p-2">
                              <div className="flex items-baseline justify-between gap-2">
                                <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-500">{metric.label}</span>
                                <span className="text-[10px] font-mono text-zinc-200">{metric.value}</span>
                              </div>
                              {history.length > 1 ? (
                        ...
                        <div className="h-full rounded border border-zinc-700/40 bg-zinc-900/30 grid place-items-center text-[9px] text-zinc-600">
                    ...
                    <div className="flex-1 min-h-0 overflow-hidden space-y-1">
                      {pagedLogs.length > 0 ? (
                        pagedLogs.map((log, index) => (
                          <div key={`${log.timestamp}:${index}`} className="rounded border border-zinc-700/40 bg-zinc-900/40 p-2">
