...
import { GitBranch, Plus, Radio, RefreshCcw, Signal, Users } from 'lucide-react';
      ...
      <section className="flex-1 min-h-0 rounded border border-zinc-700/40 bg-gradient-to-br from-zinc-900/50 to-zinc-950/80 relative overflow-hidden">
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 w-full h-full">
          ...
          {edges.map((edge) => {
            ...
            const width = 0.5 + (edge.intensity || 0) * 1.5;
            const opacity = visible ? 0.25 + (edge.intensity || 0) * 0.6 : 0;
            return (
              <line
                key={edge.id}
                x1={source.x}
        ...
        {nodes.map((node, index) => {
          ...
          const sizePx = Math.round(24 + (node.size || 0) * 0.5);
          ...
          return (
            ...
            >
              <button
                ...
                title={`${node.label}${node.type === 'channel' || node.type === 'voice' ? ` · activity ${Math.round((node.intensity || 0) * 100)}%` : ''}`}
              >
                <div
                ...
                >
                  ...
                  {(node.type === 'channel' || node.type === 'voice') && node.intensity > 0 ? (
                    <div className="text-[8px] text-zinc-300">{Math.round((node.intensity || 0) * 100)}%</div>
                  ) : null}
                </div>
              </button>
            </AnimatedMount>
          ...
          <div className="flex items-center justify-between gap-2">
            <div>
              ...
              <div className="text-[9px] text-zinc-500">
                Type: {selectedNode.type} · Activity: {Math.round((selectedNode.intensity || 0) * 100)}%
              </div>
            </div>
