...
export default function TacticalMapPanel({
  ...
  const applyMapMode = (nextMode: TacticalMapMode) => {
    ...
    if (!allowedDocks.includes(activeDockId)) {
      setActiveDockId(allowedDocks[0]);
    }
    ...
    const applyLayerPreset = (config) => {
    setLayers((prev) => prev.map((layer) => ({ ...layer, enabled: config[layer.id] !== undefined ? config[layer.id] : layer.enabled })));
    };
  ...
  const layersTab = (
    <MapLayerManager layers={layers} onToggleLayer={toggleLayer} onApplyPreset={applyLayerPreset} />
  );
  ...
  const dockTabs: MapDockTab[] = [
    ...
    { id: 'LAYERS', label: 'Layers', count: activeLayerCount, content: layersTab },
    { id: 'LEGEND', label: 'Legend', count: undefined, content: legendTab },
  ];
