      ...
      <div className="p-4 space-y-4 flex-shrink-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1 min-h-0 pb-4">
          ...
          >
            ...
            {/* Mission Dashboard */}
            <div className="border border-zinc-800/50 bg-zinc-950/50">
              <div className="px-3 py-2 border-b border-zinc-800 bg-zinc-900/30 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Target className="w-3.5 h-3.5 text-blue-400" />
                  <span className="text-[9px] font-bold uppercase text-zinc-400 tracking-wider">MISSION DASHBOARD</span>
                </div>
                <Button 
                  onClick={() => navigate(createPageUrl('Events'))}
                  variant="ghost"
                  size="sm"
                  className="text-[8px] h-5 px-2 gap-1 hover:bg-blue-900/20 hover:text-blue-400"
                >
                  FULL VIEW
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-emerald-950/30 border border-emerald-900/30 p-2">
                    <div className="text-[7px] text-emerald-400/70 uppercase mb-0.5">Active</div>
                    <div className="text-lg font-bold text-emerald-300">{userEvents.filter(e => e.status === 'active').length}</div>
                  </div>
                  <div className="bg-blue-950/30 border border-blue-900/30 p-2">
                    <div className="text-[7px] text-blue-400/70 uppercase mb-0.5">Scheduled</div>
                    <div className="text-lg font-bold text-blue-300">{userEvents.filter(e => e.status === 'scheduled').length}</div>
                  </div>
                  <div className="bg-yellow-950/30 border border-yellow-900/30 p-2">
                    <div className="text-[7px] text-yellow-400/70 uppercase mb-0.5">Pending</div>
                    <div className="text-lg font-bold text-yellow-300">{userEvents.filter(e => e.status === 'pending').length}</div>
                  </div>
                </div>
                ...
                {userEvents.length === 0 ? (
                  <div className="text-center py-8 text-zinc-600">
                    ...
                    <div className="text-[9px] uppercase">No Missions Assigned</div>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <div className="text-[8px] font-bold uppercase text-zinc-500 tracking-wider">Active & Upcoming</div>
                    {userEvents.filter(e => ['active', 'scheduled', 'pending'].includes(e.status)).slice(0, 4).map((event) => {
                      const startTime = new Date(event.start_time);
                      const timeUntil = Math.floor((startTime - new Date()) / 1000 / 60);
                      ...
                      return (
                        <div 
                          ...
                          className="p-2.5 bg-zinc-900/40 border border-zinc-800/50 hover:border-blue-500/50 transition-all cursor-pointer group"
                        >
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex-1 min-w-0">
                              <div className="text-[10px] font-bold text-zinc-200 group-hover:text-blue-400 transition-colors truncate mb-1">
                                {event.title}
                              </div>
                              <div className="flex items-center gap-1.5 flex-wrap">
                                <Badge className={cn(
                                  'text-[7px] font-bold',
                                  event.status === 'active' && 'bg-emerald-900/40 text-emerald-300 border-emerald-800',
                                  event.status === 'scheduled' && 'bg-blue-900/40 text-blue-300 border-blue-800',
                                  event.status === 'pending' && 'bg-yellow-900/40 text-yellow-300 border-yellow-800'
                                )}>{event.status}</Badge>
                                <Badge variant="outline" className="text-[7px]">{event.priority}</Badge>
                                <Badge variant="outline" className="text-[7px]">{event.event_type}</Badge>
                              </div>
                            </div>
                            <ChevronRight className="w-3.5 h-3.5 text-zinc-600 group-hover:text-blue-500 transition-colors shrink-0 ml-2" />
                          </div>

                          <div className="grid grid-cols-3 gap-2 pt-2 border-t border-zinc-800/30">
                            <div>
                              <div className="text-[7px] text-zinc-500 uppercase mb-0.5">Time</div>
                              <div className={cn("text-[8px] font-mono font-bold", isImmediate ? "text-[#ea580c]" : "text-zinc-300")}>
                                {event.status === 'active' ? 'ACTIVE' : isImmediate ? 'IMMEDIATE' : timeUntil > 0 ? `T-${timeUntil}m` : new Date(event.start_time).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                              </div>
                            </div>
                            <div>
                              <div className="text-[7px] text-zinc-500 uppercase mb-0.5">Location</div>
                              <div className="text-[8px] font-mono text-zinc-300 truncate">{event.location || 'TBD'}</div>
                            </div>
                            <div>
                              <div className="text-[7px] text-zinc-500 uppercase mb-0.5">Personnel</div>
                              <div className="text-[8px] font-mono text-zinc-300">{event.assigned_user_ids?.length || 0}</div>
                            </div>
                          </div>
                        </div>
                      );
